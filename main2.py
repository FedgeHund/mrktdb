...
for company in companies:
    ...
    for quarterlyholding in quarterlyholdings:
        ...
        for quarterlysecurityholding in quarterlysecurityholdings:
            ...
            weight_percent = (quarterlysecurityholding.marketvalue/previous_total_market_value)*100

            try:
                if(QuarterlySecurityHolding.objects.filter(quarterlyHoldingId=next_in_order(quarterlyholding),securityId = edgarSecurity.objects.filter(securityName=quarterlysecurityholding.securityId.securityName,titleOfClass = quarterlysecurityholding.securityId.titleOfClass)).first()!=None):
                    previous_weight_percent = (QuarterlySecurityHolding.objects.filter(quarterlyHoldingId=next_in_order(quarterlyholding),securityId = edgarSecurity.objects.filter(securityName=quarterlysecurityholding.securityId.securityName,titleOfClass = quarterlysecurityholding.securityId.titleOfClass)).first().marketValue/previous_total_market_value)*100
            except:
            ...
            try:
                if(prev_quarter_id_e!=None):
                    prev_quarterly_security_holding_id_e = QuarterlySecurityHolding.objects.filter(quarterlyHoldingId = prev_quarter_id_e, securityId = edgarSecurity.objects.filter(securityName=quarterlysecurityholding.securityId.securityName,titleOfClass = quarterlysecurityholding.securityId.titleOfClass) ).first()
                    print(prev_quarterly_security_holding_id_e)
                    try:
                        if(prev_quarterly_security_holding_id_e!=None):
