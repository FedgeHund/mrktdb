...
def bulk_get_or_create(prices_dict_to_create):
    for price_dict in prices_dict_to_create:
        Price.objects.get_or_create(
            securityId=price_dict.get('securityId'),
            value=price_dict.get('value'),
            quarter=price_dict.get('quarter'),
            cusip=price_dict.get('cusip'),
...
def calculate_price(security):
    logger.info(
        "Starting price calculation for security: %1s|%2s ",
        security.securityName,
        security.cusip)
    totals_per_quarter = QuarterlySecurityHolding.objects.filter(
        securityId=security).values('quarterlyHoldingId__quarter').annotate(
    ...
    for total_per_quarter in totals_per_quarter:
        ...
        price = {
            'securityId': security,
            'value': total_value / total_quantity,
            'quarter': quarter,
...
def main(argv):
    ...
    Parallel(
        ...
        calculate_price(
            security=security) for security in securities)
